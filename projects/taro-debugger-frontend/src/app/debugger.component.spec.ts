...
/**
 * Unit tests for DebuggerComponent panel layout state (WI-69).
 *
 * Verifies that all 5 InspectionPanel states (2 left, 3 right) are initialized
 * correctly per the Flush IDE layout contract defined in docs/ui-layout-inspection-panels.md.
 * Uses the class-level injection strategy to avoid rendering the full template.
 */
describe('DebuggerComponent — Panel Layout (WI-69)', () => {
  let component: DebuggerComponent;
    ...
    TestBed.configureTestingModule({
      providers: [
        DebuggerComponent,
        {
          ...
          useValue: {
            connectionStatus$: EMPTY,
            executionState$: new BehaviorSubject<string>('idle').asObservable(),
            onEvent: () => EMPTY,
            onTraffic$: EMPTY,
            disconnect: vi.fn(),
            fileTree: { readFile: () => of('') }
  ...
  it('should initialize all left sidenav panels as expanded by default', () => {
    // Arrange — component freshly created in beforeEach

    // Assert — 2 left panels: Files and Threads
    expect(component.leftFilesExpanded).toBe(true);
    expect(component.leftThreadsExpanded).toBe(true);
  });

  it('should initialize all right sidenav panels as expanded by default', () => {
    // Arrange — component freshly created in beforeEach

    // Assert — 3 right panels: Breakpoints, Variables, Call Stack
    expect(component.rightBreakpointsExpanded).toBe(true);
    expect(component.rightVariablesExpanded).toBe(true);
    expect(component.rightCallStackExpanded).toBe(true);
  });

  it('should initialize left panel heights within valid range', () => {
    // Assert — default heights must be positive and non-zero
    expect(component.leftFilesHeight).toBeGreaterThan(0);
    expect(component.leftThreadsHeight).toBeGreaterThan(0);
  });

  it('should initialize right panel heights within valid range', () => {
    // Assert — default heights must be positive and non-zero
    expect(component.rightBreakpointsHeight).toBeGreaterThan(0);
    expect(component.rightVariablesHeight).toBeGreaterThan(0);
