...
* **元件呈現**：採用 mat-nav-list 展示專案資料夾與檔案階層。由 DAP Session 直接發送 `loadedSources` 請求以取得 DAP Server 已載入之原始碼列表並建構檔案樹，檔案內容讀取則利用 `source` 請求即時取得。檔案樹資料存取服務之生命週期隨偵錯會話一同建立與銷毀，以確保檔案資料始終與當前偵錯目標保持一致。
...
| `threads` | 取得所有執行緒資訊 |
| `loadedSources` | 取得 DAP Server 載入之所有原始碼檔案清單，用於建構專案檔案樹 |
| `source` | 取得指定路徑或參照之原始碼內容 |
...
* **適用情境**：桌面單機環境或直接與本地 DAP 程序介接之場景。  
* **檔案存取**：統一透過 DAP 通訊管道（`loadedSources` 與 `source` 請求）向 DAP 伺服器取得專案或原始碼資料。  
...
* **檔案存取**：統一透過 DAP 通訊管道（`loadedSources` 與 `source` 請求）向遠端 DAP 伺服器取得專案或原始碼資料。  
...
|---|---|---|
| 原始碼檔案存取 | ✅ DAP (`loadedSources` / `source`) | ✅ DAP (`loadedSources` / `source`) |
